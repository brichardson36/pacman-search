...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        self.visited_corner = [False] * 4
        self.initial = (self.startingPosition, self.visited_corner)
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.EAST, Directions.WEST, Directions.NORTH, Directions.SOUTH]:
            ...
            "*** YOUR CODE HERE ***"
            currPos, visitedCorner = state
            x, y = currPos
            dx, dy = Actions.directionToVector(action)
            nextX, nextY = int(x + dx), int(y + dy)
            visitedCorner1 = visitedCorner[:]
            if not self.walls[nextX][nextY]:
                cornerCount = 0
                for corner in self.corners:
                    ...
                    cornerCount += 1
                if cornerCount < 4:
                    visitedCorner1[cornerCount] = True
                nextState = ((nextX, nextY), visitedCorner1)
...
def cornersHeuristic(state, problem):
    ...
    while len(unvisitedCorners) > 0:
        dist, corner = min([(util.manhattanDistance(currPos, corner), corner) for corner in unvisitedCorners])
        total += dist
        currPos = corner
