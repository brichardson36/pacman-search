...
class ValueIterationAgent(ValueEstimationAgent):
        ...
        for item in range(iterations):
          ...
          for state in states:
            ...
            for action in self.mdp.getPossibleActions(state):
              ...
              if value == None or value < currentValue:
                value = currentValue
    ...
    def computeQValueFromValues(self, state, action):
        ...
        "*** YOUR CODE HERE ***"
        value = 0
        transF = self.mdp.getTransitionStatesAndProbs(state,action)

        for nState, prob in transF:
          value += prob * (self.mdp.getReward(state, action, nState) + (self.discount * self.values[nState]))
