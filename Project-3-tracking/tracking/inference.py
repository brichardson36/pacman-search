...
class ExactInference(InferenceModule):
        ...
        if noisyDistance == None:
            ...
            for pos in self.legalPositions:
                ...
                if emissionModel[dist] > 0:
                    allPossible[pos] = self.beliefs[pos] * emissionModel[dist]
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for pos in self.legalPositions:
            agState = self.setGhostPosition(gameState, pos)
            gMoves = self.getPositionDistribution(agState)

            for newPos, p in gMoves.items():
                allPossible[newPos] += p * self.beliefs[pos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        while numParticles > 0:
            ...
            if numParticles > len(self.legalPositions):
                pLoc += self.legalPositions
                numParticles      -= len(self.legalPositions)

            else:
                pLoc += self.legalPositions[0:numParticles]
                numParticles = 0
    ...
    def observe(self, observation, gameState):
        ...
        allPossible = util.Counter()
        bDist = self.getBeliefDistribution()
        self.particles = []
        ...
        if noisyDistance == None:
            ...
            for p in self.legalPositions:
                ...
                allPossible[p] += emissionModel[d] * bDist[p]
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            pos.append(util.sample(newPosDist))
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for particle in self.particles:
            ...
            for cGhost in range(self.numGhosts):

                if noisyDistances[cGhost] == None:
                    particle = self.getParticleWithGhostInJail(particle, cGhost)

                else:
                    dGhost.append(cGhost)

            pProb = 1

            for cfGhost in dGhost:
                model = emissionModels[cfGhost]
                gPos = particle[cfGhost]
                dist = util.manhattanDistance(gPos, pacmanPosition)
                pProb *= model[dist]

            allPossible[particle] += pProb
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            for cGhost in range(self.numGhosts):
                altered_gamestate = setGhostPositions(gameState, oldPositions)
                ghostAgent = self.ghostAgents[cGhost]

                positionDist = getPositionDistributionForGhost(altered_gamestate, cGhost, ghostAgent)
                newParticle[cGhost] = util.sample(positionDist)
